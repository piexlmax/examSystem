<template>
  <div>
    <div class="search-term">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        <el-form-item label="考题">
          <el-select
            v-model="searchInfo.testPaperId"
            @change="onSubmit"
            filterable
            placeholder="请选择考题"
          >
    ...
    >
      ...
      <el-table-column label="同意协议" prop="agreement" width="120">
        <template slot-scope="scope">
          <div>{{scope.row.agreement?"已同意":"未同意"}}</div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="答题状况" prop="testPath" width="120">
        <template slot-scope="scope">
          <div>
            <el-button v-if="scope.row.testPath" @click="downloadTestPaperSvgNode(scope.row)">下载答卷</el-button>
    ...
    <el-dialog title="评阅" :visible.sync="dialogVisible" width="30%">
      <el-form ref="elForm" :model="formData" :rules="rules" size="medium" label-width="100px">
        <el-form-item label="在线审阅">
          <el-button type="primary" @click="viewTest">审阅</el-button>
        </el-form-item>
    ...
    <el-dialog 
    title="预览"
    :visible.sync="fullPageVisible"
    fullscreen
    >
    <div v-html="html"></div>
    <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="fullPageVisible=false">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
import { downloadTestPaperSvgNode, getTestPaperList } from "@/api/testPaper";
...
export default {
  ...
  methods: {
    async viewTest() {
      const that = this
      if (this.activeRecordPath.indexOf(".docx") > -1) {
        const res = await downloadTestPaperSvgNode({
          path: this.activeRecordPath
        });
        ...
        window.open(
          origin +
            ...
            this.activeRecordId +
            "/" +
            this.activeRecordName
        );
      }
    },
    async offlineAppraise() {
      const res = await offlineAppraise({
        testPaperId: String(this.searchInfo.testPaperId)
      });
      const blob = new Blob([res]);
    ...
    onSubmit(index) {
      this.activeTestName = this.options.filter(
    ...
    openAppraise(row) {
      ...
      this.activeRecordName = row.testPath.split(
        "/" + row.testPaperId + "/"
      )[1];
      this.activeRecordPath = row.testPath;
      this.activeRecordId = row.testPaperId;
